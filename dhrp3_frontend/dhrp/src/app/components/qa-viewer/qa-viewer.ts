import { Component, OnInit } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { marked } from 'marked';
 
// ✅ Angular Material imports
import { MatFormFieldModule } from '@angular/material/form-field';
import { MatSelectModule } from '@angular/material/select';
import { MatOptionModule } from '@angular/material/core';
import { MatIconModule } from '@angular/material/icon';
import { MatInputModule } from '@angular/material/input';
import { MatButtonModule } from '@angular/material/button';
 
@Component({
  selector: 'app-qa-viewer',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    MatFormFieldModule,
    MatSelectModule,
    MatOptionModule,
    MatIconModule,
    MatInputModule,
    MatButtonModule
  ],
  templateUrl: './qa-viewer.html',
  styleUrl: './qa-viewer.css'
})
export class QaViewer implements OnInit {
  filename: string = '';
  rows: any[] = [];
  columns: string[] = [];
  searchTerm: string = '';
  loading: boolean = true;
  activeItemId: number | null = null;
  baseName: string = '';

 
  constructor(private route: ActivatedRoute, private router: Router) {}
 
  ngOnInit(): void {
    const doc = this.route.snapshot.paramMap.get('doc') || '';
    this.filename = doc.endsWith('.pdf') ? doc.replace('.pdf', '_analysis.csv') : doc;
 
    const url = `http://localhost:5000/view_csv/${this.filename}`;
    fetch(url)
      .then(res => res.json())
      .then(async data => {
        if (data.success && Array.isArray(data.rows)) {
          this.columns = Array.isArray(data.columns) ? data.columns : [];
          this.rows = await Promise.all(
            data.rows.map(async (row: Record<string, any>, index: number) => {
              const answer = row['Answer'] || '';
              const html = marked.parse(answer) as string;
              return { ...row, AnswerHtml: html, id: index + 1, status: 'pending' };
            })
          );
          this.baseName = data.base_name || this.filename.replace('_analysis.csv', '');
        } else {
          alert(data.message || 'Unexpected CSV format.');
        }
        this.loading = false;
      })
      
      .catch(err => {
        console.error('Fetch error:', err);
        alert('Failed to load Q/A data.');
        this.loading = false;
      });
  }
 
  goBack(): void {
    this.router.navigate(['/']);
  }
 
  get filteredRows(): any[] {
    const term = this.searchTerm.toLowerCase();
    return this.rows.filter(row =>
      this.columns.some(col =>
        String(row[col] || '').toLowerCase().includes(term)
      )
    );
  }
 
  downloadCSV(): void {
    const downloadUrl = `http://localhost:5000/download/${this.filename}`;
    window.open(downloadUrl, '_blank');
  }
 
  selectItem(id: number): void {
    this.activeItemId = id;
    this.loadComments(id);
  }
 
  getStatusIcon(status: string): string {
    switch (status) {
      case 'compliant': return 'check_circle';
      case 'partial': return 'warning';
      case 'non-compliant': return 'cancel';
      case 'not-applicable': return 'do_not_disturb';
      default: return 'pending';
    }
  }
 
  getStatusColor(status: string): string {
    switch (status) {
      case 'compliant': return 'success';
      case 'partial': return 'warning';
      case 'non-compliant': return 'error';
      case 'not-applicable': return 'disabled';
      default: return 'pending';
    }
  }
  submitComment(item: any): void {
    const payload = {
      itemId: item.id,
      filename: this.filename,
      comment: item.newComment,
      reviewer: 'Amal' // or dynamic user
    };
 
    fetch('http://localhost:5000/api/comments', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          console.log('✅ Comment saved:', data.comment);
          item.newComment = '';
        } else {
          alert('Failed to save comment.');
        }
      })
      .catch(err => {
        console.error('❌ Comment error:', err);
        alert('Error posting comment.');
      });
  }
  comments: any[] = [];
 
loadComments(itemId: number): void {
  const url = `http://localhost:5000/api/comments?filename=${this.filename}&itemId=${itemId}`;
  fetch(url)
    .then(res => res.json())
    .then(data => {
      this.comments = Array.isArray(data.comments) ? data.comments : [];
    })
    .catch(err => {
      console.error('❌ Failed to load comments:', err);
      this.comments = [];
    });
}
newComment: string = '';
 
submitCommentFromRightPanel(): void {
  const item = this.filteredRows.find(i => i.id === this.activeItemId);
  if (item && this.newComment?.trim()) {
    const payload = {
      itemId: item.id,
      filename: this.filename,
      comment: this.newComment,
      reviewer: 'John'
    };
 
    fetch('http://localhost:5000/api/comments', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          console.log('✅ Comment saved:', data.comment);
          this.newComment = '';
          this.loadComments(item.id); // ✅ Refresh after successful post
        } else {
          alert('Failed to save comment.');
        }
      })
      .catch(err => {
        console.error('❌ Comment error:', err);
        alert('Error posting comment.');
      });
  }
}
expandedItemId: number | null = null;

toggleItem(id: number): void {
  if (this.expandedItemId === id) {
    this.expandedItemId = null; // collapse if already open
  } else {
    this.expandedItemId = id;   // expand this item
  }
}

 
}