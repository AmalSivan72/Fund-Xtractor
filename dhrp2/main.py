...
def process_dhrp(base):
    try:
        ...
        if not entry:
            ...
            return jsonify({"success": False, "message": "Entry not found"}), 404
        if not entry.toc_verified:
            logging.warning(f"⚠️ TOC not verified for: {filename}")
            return jsonify({"success": False, "message": "TOC not verified. Please verify before processing."}), 400
...
def get_toc(doc):
    try:
        ...
        if not sections:
            logging.warning(f"⚠️ No TOC sections found in DB for: {doc}")
            return jsonify({"toc": []})
        toc_dict = {}
        ...
        logging.info(f"📄 TOC served from DB for: {doc} — Sections: {len(toc_list)}")
        return jsonify({"toc": toc_list})
    except Exception as e:
        logging.error(f"❌ Error retrieving TOC from DB for {doc}: {str(e)}")
